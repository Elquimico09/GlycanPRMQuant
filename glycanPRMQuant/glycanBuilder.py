...
def get_path_nodes(G, source, target):
    ...
    try:
        # shortest_path is appropriate for DAGs like glycans
        return nx.shortest_path(G, source=source, target=target)
    except nx.NetworkXNoPath:
        # print(f"Warning: No path found from {source} to {target}")
        return None
    except nx.NodeNotFound:
        # print(f"Warning: Node {source} or {target} not found in graph.")
        return None
...
def are_edges_on_different_branches(G, edge1, edge2, root_node):
    ...
    A branch point is defined as a node with out_degree > 1.

    Parameters:
        G: The original NetworkX DiGraph glycan structure.
        edge1: Tuple representing the first edge (u1, v1).
        edge2: Tuple representing the second edge (u2, v2).
        root_node: The root node of the glycan graph.

    Returns:
        True if the edges are determined to be on different branches, False otherwise.
    """
    u1, _ = edge1
    u2, _ = edge2

    # If the parent nodes are the same, they must stem from the same branch point (if it is one)
    if u1 == u2:
        # Check if the common parent is a branch point
        return G.out_degree(u1) > 1

    # Find paths from root to the parent node of each cut edge
    path1_nodes = get_path_nodes(G, root_node, u1)
    path2_nodes = get_path_nodes(G, root_node, u2)

    # If either path doesn't exist, cannot determine; treat as same branch for safety
    if path1_nodes is None or path2_nodes is None:
        print(f"Warning: Could not determine paths for branch check on edges {edge1}, {edge2}.")
        return False

    # Find the last common ancestor node by comparing paths element-wise
    last_common_node = None
    len1, len2 = len(path1_nodes), len(path2_nodes)
    idx = 0
    while idx < len1 and idx < len2 and path1_nodes[idx] == path2_nodes[idx]:
        last_common_node = path1_nodes[idx]
        idx += 1

    # If paths didn't even share the root (unexpected), return False
    if last_common_node is None:
        print(f"Warning: Paths for edges {edge1}, {edge2} did not share root {root_node}.")
        return False

    # Check if one path is simply a continuation of the other
    # If the last common node is the parent of one of the edges, they are on the same branch path
    if last_common_node == u1 or last_common_node == u2:
        return False

    # If paths diverge *after* the last_common_node, check if that node is a branch point
    if G.out_degree(last_common_node) > 1:
        # Check that they actually diverge (next nodes are different)
        node_after_common1 = path1_nodes[idx] if idx < len1 else None
        node_after_common2 = path2_nodes[idx] if idx < len2 else None
        if node_after_common1 != node_after_common2:
             return True # Diverged at a branch point
        else:
             # Paths share same next node after common ancestor? Unexpected structure.
             return False
    else:
        # Diverged, but not at a node with out_degree > 1. Treat as same branch.
        return False
...
def fragment_glycans_yy(G, permethylation=False):
    """
    Fragments glycans keeping y-like ions, using layer-by-layer and cross-layer cuts.
    Applies specific mass adjustments based on cut type:
    - Standard y-ion (1 cut, or 2 cuts on same branch): raw_mass - 14.0156
    - Multi-branch 2-cut y-ion: raw_mass - (2 * 14.0156)

    Parameters & Returns: (Same as before)
    """
    fragments = {}
    unique_fragments_tracker = {}
    if not G.nodes: return fragments

    # --- Setup: Root Node and Layers ---
    try:
        root_nodes = [n for n in G.nodes() if G.in_degree(n) == 0]
        if len(root_nodes) != 1:
             if 1 in G.nodes and G.in_degree(1) == 0: root_node = 1
             else: raise ValueError(f"Cannot determine unique root node. Found: {root_nodes}")
        else: root_node = root_nodes[0]
    except Exception as e: raise ValueError(f"Error finding root node: {e}")

    all_layers = []
    for node, data in G.nodes(data=True):
        if 'layer' not in data: raise ValueError(f"Node {node} missing 'layer'.")
        all_layers.append(data['layer'])
    if not all_layers: return fragments
    max_layer = max(all_layers)
    # --- End Setup ---

    y_ion_1_cut_adjustment = -14.0156
    y_ion_2_cut_multi_branch_adjustment = - (2 * 14.0156)

    # === Part 1: Layer-by-Layer Fragmentation ===
    print("Starting layer-by-layer fragmentation (y-like ions)...")
    layer_fragments_added = 0
    for layer in range(max_layer, 0, -1):
        layer_edges = [(u, v) for u, v in G.edges() if G.nodes[v]['layer'] == layer]
        if not layer_edges: continue
        edge_combinations_to_process = []
        if len(layer_edges) == 1: edge_combinations_to_process.append(tuple(layer_edges))
        elif len(layer_edges) >= 2:
             if len(layer_edges) == 2: edge_combinations_to_process.append(tuple(layer_edges))
             else: edge_combinations_to_process = list(itertools.combinations(layer_edges, 2))

        for cut_edge_set_tuple in edge_combinations_to_process:
            cut_edge_set = set(cut_edge_set_tuple); actual_cuts = 0 # Reset actual_cuts here
            if not cut_edge_set: continue
            G_temp = G.copy()
            existing_edges_to_cut = [edge for edge in cut_edge_set if G_temp.has_edge(*edge)]
            actual_cuts = len(existing_edges_to_cut) # Set actual_cuts based on found edges
            if not existing_edges_to_cut: continue
            G_temp.remove_edges_from(existing_edges_to_cut)
            root_component_nodes = None
            try:
                for component in nx.weakly_connected_components(G_temp):
                    if root_node in component: root_component_nodes = component; break
            except Exception: continue

            if root_component_nodes:
                y_fragment_graph = G.subgraph(root_component_nodes).copy()
                if len(y_fragment_graph.nodes()) <= 1: continue
                ...
                if y_unique_key not in unique_fragments_tracker:
                    ...
                    # --- End Fix ---
                    composition_tuple = (node_types.get("HexNAc", 0), node_types.get("Hex", 0), node_types.get("Fuc", 0), node_types.get("Neu5Ac", 0))
                    raw_mass = calculate_mass(composition_tuple, permethylation)

                    mass_adjustment = 0.0; is_multi_branch_cut = False; effective_cuts = actual_cuts
                    if actual_cuts == 1:
                        mass_adjustment = y_ion_1_cut_adjustment; effective_cuts = 1
                    elif actual_cuts == 2:
                        edge_list = list(existing_edges_to_cut)
                        is_multi_branch_cut = are_edges_on_different_branches(G, edge_list[0], edge_list[1], root_node)
                        if is_multi_branch_cut:
                            mass_adjustment = y_ion_2_cut_multi_branch_adjustment; effective_cuts = 2
                        else: mass_adjustment = y_ion_1_cut_adjustment; effective_cuts = 1

                    final_mass = raw_mass + mass_adjustment

                    base_id = f"yL{layer}-{y_composition_str}"
                    count = 1; fragment_id = f"{base_id}-{count}"
                    while fragment_id in fragments: count += 1; fragment_id = f"{base_id}-{count}"
                    fragments[fragment_id] = {
                        "graph": y_fragment_graph, "mass": final_mass, "composition": y_composition_str,
                        "type": "y", "cuts": actual_cuts, "multi_branch_cut": is_multi_branch_cut,
                        "effective_cuts": effective_cuts, "cut_edges": set(existing_edges_to_cut),
                        "cut_layer": layer
                    }
                    unique_fragments_tracker[y_unique_key] = fragment_id
                    layer_fragments_added += 1
    print(f"Added {layer_fragments_added} unique fragments from layer-by-layer method.")

    # === Part 2: Cross-Layer Double Fragmentation (Layers >= 2) ===
    print("\nStarting cross-layer double fragmentation (yy-like ions)...")
    cross_layer_fragments_added = 0
    edges_by_layer = {}
    for u, v in G.edges():
        try: target_layer = G.nodes[v]['layer']
        except KeyError: continue
        if target_layer >= 2:
            if target_layer not in edges_by_layer: edges_by_layer[target_layer] = []
            edges_by_layer[target_layer].append((u, v))
    involved_layers = sorted(edges_by_layer.keys())

    for i in range(len(involved_layers)):
        for j in range(i + 1, len(involved_layers)):
            layer1 = involved_layers[i]; layer2 = involved_layers[j]
            if layer1 not in edges_by_layer or layer2 not in edges_by_layer: continue
            for edge1 in edges_by_layer[layer1]:
                for edge2 in edges_by_layer[layer2]:
                    cut_edge_set = {edge1, edge2}
                    G_temp = G.copy()
                    existing_edges_to_cut = [edge for edge in cut_edge_set if G_temp.has_edge(*edge)]
                    actual_cuts = len(existing_edges_to_cut)
                    if actual_cuts != 2: continue
                    G_temp.remove_edges_from(existing_edges_to_cut)
                    root_component_nodes = None
                    try:
                         for component in nx.weakly_connected_components(G_temp):
                    ...
                    if root_component_nodes:
                        yy_fragment_graph = G.subgraph(root_component_nodes).copy()
                        if len(yy_fragment_graph.nodes()) <= 1: continue

                        yy_composition_str = get_fragment_composition(yy_fragment_graph)
                        yy_nodes_tuple = tuple(sorted(list(yy_fragment_graph.nodes())))
                        yy_unique_key = ("y", yy_composition_str, yy_nodes_tuple)

                        if yy_unique_key not in unique_fragments_tracker:
                            # --- Fix for UnboundLocalError ---
                            node_types = {}
                            for _, data in yy_fragment_graph.nodes(data=True):
                                node_type = data["type"]
                                node_types[node_type] = node_types.get(node_type, 0) + 1
                             # --- End Fix ---
                            composition_tuple = (node_types.get("HexNAc", 0), node_types.get("Hex", 0), node_types.get("Fuc", 0), node_types.get("Neu5Ac", 0))
                            ...
                            base_id = f"yL{layer1}+L{layer2}-{yy_composition_str}"
                            count = 1; fragment_id = f"{base_id}-{count}"
                            while fragment_id in fragments: count += 1; fragment_id = f"{base_id}-{count}"
                            fragments[fragment_id] = {
                                "graph": yy_fragment_graph, "mass": final_mass, "composition": yy_composition_str,
                                "type": "y", "cuts": actual_cuts, "multi_branch_cut": is_multi_branch_cut,
                                "effective_cuts": effective_cuts, "cut_edges": set(existing_edges_to_cut),
                                "cut_layer": (layer1, layer2)
                            }
                            unique_fragments_tracker[yy_unique_key] = fragment_id
                            cross_layer_fragments_added += 1
    print(f"Added {cross_layer_fragments_added} unique fragments from cross-layer method.")
    print(f"Total unique y-like fragments found: {len(fragments)}")
    return fragments
...
def are_edges_on_different_branches(G, edge1, edge2, root_node):
    """
    Improved function to check if two edges originate from different branches relative to the root.
    ...
    # If the parent nodes are the same, check if this is a branch point
    if u1 == u2:
        return G.out_degree(u1) > 1
    
    # Find paths from root to each target node (v1 and v2)
    path1 = nx.shortest_path(G, source=root_node, target=v1)
    path2 = nx.shortest_path(G, source=root_node, target=v2)
    ...
    # If the paths diverged, get the branch point
    if common_prefix_length > 0 and common_prefix_length < min_len:
        branch_point = path1[common_prefix_length - 1]
        # Verify this is actually a branch point (has multiple outgoing edges)
        return G.out_degree(branch_point) > 1
